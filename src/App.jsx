...
export default function App(){
  ...
  const [modal,setModal]=useState(null);// {type, spec, qId}
  ...
  const [editCount,setEditCount]=useState(null);// {specId, qId, val}
  ...
  // +1 manual com motivo (qualquer usuário)
  async function addManual(qId,specId,motivo){
    ...
    try{
      ...
      await sb("history","POST",{spec_name:spec.name,queue_id:qId,type:"manual",by_user:userName,date_key:today,detail:motivo});
      showToast(`+1 registrado: ${spec.name}`);
      const sp=await sb("specialists?order=name");if(sp?.length)setSpecs(sp);
      const hi=await sb("history?order=created_at.desc&limit=1000");if(hi?.length)setHist(hi);
    }catch{showToast("Erro.","error");}
  }

  // Admin: editar contador diretamente
  async function saveEditCount(){
    if(!editCount)return;
    const spec=specs.find(c=>c.id===editCount.specId);if(!spec)return;
    const val=parseInt(editCount.val);if(isNaN(val)||val<0)return;
    try{
      await sb(`specialists?id=eq.${spec.id}`,"PATCH",{counts:{...spec.counts,[editCount.qId]:val}});
  ...
  async function setVacation(spec,on,note=""){try{await sb(`specialists?id=eq.${spec.id}`,"PATCH",{status:on?"vacation":"active",note:on?note:spec.note});await sb("events","POST",{type:on?"ferias_inicio":"ferias_fim",spec_name:spec.name,detail:on?note:"Retornou de férias",by_user:userName,date_key:today});showToast(on?"Férias registradas!":"Retorno registrado!");const sp=await sb("specialists?order=name");if(sp?.length)setSpecs(sp);const ev=await sb("events?order=created_at.desc&limit=500");if(ev?.length)setEvts(ev);}catch{showToast("Erro.","error");}}
  ...
  async function iniciarNovoDia(){
    ...
    const summary=QUEUES.map(q=>{const e=hist.filter(h=>h.date_key===yKey&&h.queue_id===q.id);return{queue:q.label,normal:e.filter(h=>h.type==="normal").length,ind:e.filter(h=>h.type!=="normal"&&h.type!=="recart_ferias").length,recart:e.filter(h=>h.type==="recart_ferias").length,total:e.length};}).filter(s=>s.total>0);
    try{
  ...
  // PDF mensal
  function gerarPDFMensal(y,m){
    const yk=yesterdayKey();
    const all=QUEUES.map(q=>{
      const inQq=specs.filter(c=>c.queues.includes(q.id));
      const wd=getWorkdays(y,m);
      const cnt=(n,dk,t)=>hist.filter(h=>h.spec_name===n&&h.date_key===dk&&h.queue_id===q.id&&h.type===t).length;
      const cntR=(n,dk)=>hist.filter(h=>h.spec_name===n&&h.date_key===dk&&h.queue_id===q.id&&h.type==="recart_ferias").length;
      const cntM=(n,t)=>hist.filter(h=>{if(h.spec_name!==n||h.queue_id!==q.id||h.type!==t)return false;const p=h.date_key?.split("/");return p&&parseInt(p[1])===m+1&&parseInt(p[2])===y;}).length;
      const cntMR=(n)=>hist.filter(h=>{if(h.spec_name!==n||h.queue_id!==q.id||h.type!=="recart_ferias")return false;const p=h.date_key?.split("/");return p&&parseInt(p[1])===m+1&&parseInt(p[2])===y;}).length;
      const dH=wd.map(d=>`<th colspan="3">${d.toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit"})}</th>`).join("");
      const sH=wd.map(()=>`<th class="n">N</th><th class="i">I</th><th class="rf">RF</th>`).join("");
      const rows=inQq.map(c=>{
        const tN=cntM(c.name,"normal"),tI=cntM(c.name,"indicacao")+cntM(c.name,"selecao")+cntM(c.name,"manual"),tRF=cntMR(c.name);
        const cells=wd.map(d=>{const dk=d.toLocaleDateString("pt-BR"),n=cnt(c.name,dk,"normal"),ii=cnt(c.name,dk,"indicacao")+cnt(c.name,dk,"selecao")+cnt(c.name,dk,"manual"),rf=cntR(c.name,dk);return`<td class="${n>0?"nv":"e"}">${n||"—"}</td><td class="${ii>0?"iv":"e"}">${ii||"—"}</td><td class="${rf>0?"rfv":"e"}">${rf||"—"}</td>`;}).join("");
        return`<tr><td class="name">${c.name}</td>${cells}<td class="tn">${tN||"—"}</td><td class="ti">${tI||"—"}</td><td class="trf">${tRF||"—"}</td></tr>`;
      }).join("");
      const tot=`<tr class="tot"><td class="name">TOTAL</td>${wd.map(d=>{const dk=d.toLocaleDateString("pt-BR"),n=inQq.reduce((a,c)=>a+cnt(c.name,dk,"normal"),0),ii=inQq.reduce((a,c)=>a+cnt(c.name,dk,"indicacao")+cnt(c.name,dk,"selecao")+cnt(c.name,dk,"manual"),0),rf=inQq.reduce((a,c)=>a+cntR(c.name,dk),0);return`<td class="${n>0?"nv":"e"}">${n||"—"}</td><td class="${ii>0?"iv":"e"}">${ii||"—"}</td><td class="${rf>0?"rfv":"e"}">${rf||"—"}</td>`;}).join("")}<td class="tn">${inQq.reduce((a,c)=>a+cntM(c.name,"normal"),0)||"—"}</td><td class="ti">${inQq.reduce((a,c)=>a+cntM(c.name,"indicacao")+cntM(c.name,"selecao")+cntM(c.name,"manual"),0)||"—"}</td><td class="trf">${inQq.reduce((a,c)=>a+cntMR(c.name),0)||"—"}</td></tr>`;
      return`<div class="setor"><h2>${q.icon} ${q.label}</h2><div class="tbl-wrap"><table><thead><tr><th rowspan="2" class="name">Especialista</th>${dH}<th colspan="3" class="tot-h">Total</th></tr><tr>${sH}<th class="n">N</th><th class="i">I</th><th class="rf">RF</th></tr></thead><tbody>${rows}${tot}</tbody></table></div></div>`;
    }).join("");
    const html=`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Rodízio — ${MONTHS[m]} ${y}</title><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:Arial,sans-serif;font-size:11px;padding:20px;color:#222}.header{margin-bottom:24px;border-bottom:3px solid #7C3AED;padding-bottom:12px}h1{font-size:20px;color:#7C3AED}p{color:#888;margin-top:4px}.setor{margin-bottom:32px;page-break-inside:avoid}h2{font-size:14px;color:#7C3AED;margin-bottom:8px;padding:6px 10px;background:#f5f3ff;border-radius:6px}.tbl-wrap{overflow-x:auto}table{border-collapse:collapse;width:100%;min-width:600px}th,td{border:0.5px solid #e0ddf8;padding:4px 6px;text-align:center;white-space:nowrap}th{background:#f5f3ff;color:#534AB7}.tot-h{background:#d8d4fc;color:#3C3489}.name{text-align:left;min-width:120px;font-weight:500}.n{color:#7C3AED;font-size:10px}.i{color:#F59E0B;font-size:10px}.rf{color:#0EA5E9;font-size:10px}.nv{background:#EDE9FE;color:#534AB7;font-weight:500}.iv{background:#FEF3C7;color:#B45309;font-weight:500}.rfv{background:#E0F2FE;color:#0369A1;font-weight:500}.e{color:#ccc}.tn{background:#EDE9FE;color:#534AB7;font-weight:700}.ti{background:#FEF3C7;color:#B45309;font-weight:700}.trf{background:#E0F2FE;color:#0369A1;font-weight:700}.tot td{background:#f5f3ff;font-weight:700;border-top:2px solid #C4B5F4}.legenda{margin-top:24px;font-size:10px;color:#888;border-top:1px solid #e5e7eb;padding-top:8px}@media print{.setor{page-break-after:always}}</style></head><body><div class="header"><h1>🔄 Rodízio de Especialistas</h1><p>${MONTHS[m]} ${y} · Gerado em ${new Date().toLocaleString("pt-BR")}</p></div>${all}<div class="legenda">N = Rodízio normal &nbsp;|&nbsp; I = Indicações / Seleção especial / Cliente presencial &nbsp;|&nbsp; RF = Recarteirização de férias</div></body></html>`;
  ...
  // AUTH SCREENS
  const authScreen=(icon,title,sub,content)=>(
  ...
  const pinInp=(val,set,ph,onEnter)=><input type="password" style={{width:"100%",boxSizing:"border-box",padding:"12px 16px",borderRadius:10,border:"2px solid #e5e7eb",fontSize:15,marginBottom:16,outline:"none",color:"#222",textAlign:"center",letterSpacing:4}} placeholder={ph} value={val} onChange={e=>set(e.target.value)} onKeyDown={e=>e.key==="Enter"&&onEnter()} autoFocus/>;
  const btnMain=(label,onClick)=><button style={{width:"100%",padding:"12px",borderRadius:10,border:"none",background:"linear-gradient(135deg,#7C3AED,#4F46E5)",color:"#fff",fontSize:15,fontWeight:600,cursor:"pointer",marginBottom:10}} onClick={onClick}>{label}</button>;
  const btnBack=(onClick)=><button style={{background:"none",border:"none",color:"#aaa",fontSize:13,cursor:"pointer"}} onClick={onClick}>← Voltar</button>;

  if(authStep==="name"||authStep==="idle")return authScreen("👋","Bem-vindo!","Digite seu nome para continuar",<><input style={{width:"100%",boxSizing:"border-box",padding:"12px 16px",borderRadius:10,border:"2px solid #e5e7eb",fontSize:15,marginBottom:16,outline:"none",color:"#222"}} placeholder="Seu nome" value={tmpName} onChange={e=>setTmpName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleNameSubmit()} autoFocus/>{btnMain("Continuar",handleNameSubmit)}</>);
  if(authStep==="pin_enter_admin")return authScreen("🔐",`Olá, ${tmpName}!`,"Digite o PIN de administrador",<>{pinInp(tmpPin,setTmpPin,"••••••••",handleAdminPinSubmit)}{btnMain("Entrar",handleAdminPinSubmit)}{btnBack(()=>{setAuthStep("name");setTmpPin("");})}</>);
  if(authStep==="pin_create")return authScreen("🔑",`Olá, ${tmpName}!`,"Crie um PIN para proteger seu acesso",<>{pinInp(tmpPin,setTmpPin,"Criar PIN",()=>{})}{pinInp(tmpPin2,setTmpPin2,"Confirmar PIN",handlePinCreate)}{btnMain("Criar PIN e entrar",handlePinCreate)}{btnBack(()=>{setAuthStep("name");setTmpPin("");setTmpPin2("");})}</>);
  if(authStep==="pin_enter")return authScreen("🔒",`Olá, ${tmpName}!`,"Digite seu PIN para entrar",<>{pinInp(tmpPin,setTmpPin,"••••",handlePinEnter)}{btnMain("Entrar",handlePinEnter)}{btnBack(()=>{setAuthStep("name");setTmpPin("");})}</>);
    ...
    return(
      <div style={{...C.card,borderTop:`4px solid ${q.color}`,padding:0,overflow:"hidden"}}>
        ...
        <div style={{padding:"0.75rem 1rem",borderBottom:"1px solid #f3f4f6",display:"flex",gap:8,flexWrap:"wrap"}}>
          ...
          <button style={{...C.btnS,flex:"1 1 100%",borderColor:"#0EA5E9",color:"#0EA5E9",fontWeight:600,fontSize:12}} onClick={()=>rotateRecart(qId)}>↩️ Recart. Férias</button>
        </div>
  ...
  function ControleTab(){
    ...
    const cntRF=(n,dk)=>hist.filter(h=>h.spec_name===n&&h.date_key===dk&&h.queue_id===ctrlQ&&h.type==="recart_ferias").length;
    ...
    const cntMRF=(n)=>hist.filter(h=>{if(h.spec_name!==n||h.queue_id!==ctrlQ||h.type!=="recart_ferias")return false;const p=h.date_key?.split("/");return p&&parseInt(p[1])===ctrlM.m+1&&parseInt(p[2])===ctrlM.y;}).length;
    ...
    const td={padding:"4px 6px",fontSize:12,textAlign:"center",border:"0.5px solid #f3f4f6",whiteSpace:"nowrap"};
    return(
      <div>
        <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
          ...
          <div style={{display:"flex",gap:6,marginLeft:"auto"}}>
            <button style={{...C.btnP,fontSize:12,background:"linear-gradient(135deg,#0EA5E9,#0369A1)"}} onClick={()=>gerarPDFMensal(ctrlM.y,ctrlM.m)}>📄 PDF do mês</button>
          </div>
        </div>
            ...
            <thead>
              ...
              <tr>
                {wd.map(d=>[
                  ...
                  <th key={d.toISOString()+"rf"} style={{...th,fontSize:10,color:"#0EA5E9"}}>RF</th>
                ])}
                ...
                <th style={{...th,fontSize:10,color:"#0369A1",background:"#E0F2FE"}}>RF</th>
              </tr>
            </thead>
            <tbody>
              {inQ.map((c,ri)=>{
                const rb=ri%2===0?"#fff":"#fafafa",tN=cntM(c.name,"normal"),tI=cntM(c.name,"indicacao")+cntM(c.name,"selecao")+cntM(c.name,"manual"),tRF=cntMRF(c.name);
                return(
                  <tr key={c.id}>
                    ...
                    {wd.map(d=>{const dk=d.toLocaleDateString("pt-BR"),n=cnt(c.name,dk,"normal"),ii=cnt(c.name,dk,"indicacao")+cnt(c.name,dk,"selecao")+cnt(c.name,dk,"manual"),rf=cntRF(c.name,dk);return[
                      <td key={dk+"n"} style={{...td,background:n>0?"#EDE9FE":rb,color:n>0?"#7C3AED":"#ddd",fontWeight:n>0?600:400}}>{n||""}</td>,
                      <td key={dk+"i"} style={{...td,background:ii>0?"#FEF3C7":rb,color:ii>0?"#B45309":"#ddd",fontWeight:ii>0?600:400}}>{ii||""}</td>,
                      <td key={dk+"rf"} style={{...td,background:rf>0?"#E0F2FE":rb,color:rf>0?"#0369A1":"#ddd",fontWeight:rf>0?600:400}}>{rf||""}</td>
                    ];})}
                    ...
                    <td style={{...td,fontWeight:700,background:"#E0F2FE",color:"#0369A1"}}>{tRF||""}</td>
                  </tr>
                );
              })}
              <tr style={{borderTop:`2px solid ${qInfo.color}40`}}>
                <td style={{...td,textAlign:"left",fontWeight:700,background:"#f9fafb"}}>TOTAL</td>
                {wd.map(d=>{const dk=d.toLocaleDateString("pt-BR"),n=inQ.reduce((a,c)=>a+cnt(c.name,dk,"normal"),0),ii=inQ.reduce((a,c)=>a+cnt(c.name,dk,"indicacao")+cnt(c.name,dk,"selecao")+cnt(c.name,dk,"manual"),0),rf=inQ.reduce((a,c)=>a+cntRF(c.name,dk),0);return[
                  <td key={dk+"tn"} style={{...td,fontWeight:600,background:n>0?qInfo.light:"#f9fafb",color:qInfo.color}}>{n||""}</td>,
                  <td key={dk+"ti"} style={{...td,fontWeight:600,background:ii>0?"#FEF3C7":"#f9fafb",color:"#B45309"}}>{ii||""}</td>,
                  <td key={dk+"trf"} style={{...td,fontWeight:600,background:rf>0?"#E0F2FE":"#f9fafb",color:"#0369A1"}}>{rf||""}</td>
                ];})}
                <td style={{...td,fontWeight:700,background:qInfo.light,color:qInfo.color}}>{inQ.reduce((a,c)=>a+cntM(c.name,"normal"),0)||""}</td>
                <td style={{...td,fontWeight:700,background:"#FEF3C7",color:"#B45309"}}>{inQ.reduce((a,c)=>a+cntM(c.name,"indicacao")+cntM(c.name,"selecao")+cntM(c.name,"manual"),0)||""}</td>
                <td style={{...td,fontWeight:700,background:"#E0F2FE",color:"#0369A1"}}>{inQ.reduce((a,c)=>a+cntMRF(c.name),0)||""}</td>
              </tr>
            </tbody>
          </table>
        </div>
        <div style={{marginTop:8,fontSize:12,color:"#888"}}>N = rodízio normal · I = indicações / seleção / cliente presencial · RF = recart. férias</div>
      </div>
    );
  }
              ...
              {sdrList.map((sdr,si)=>{
                ...
                return(<tr key={sdr}>
                  <td style={{padding:"8px 12px",fontWeight:600,fontSize:13,background:rb,border:"0.5px solid #f3f4f6"}}>
                    <div style={{display:"flex",alignItems:"center",gap:6}}><div style={{width:24,height:24,borderRadius:"50%",background:"#EDE9FE",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,color:"#7C3AED"}}>{initials(sdr)}</div>{sdr}{sdr===userName&&<span style={{fontSize:10,color:"#7C3AED",background:"#EDE9FE",borderRadius:6,padding:"1px 5px"}}>você</span>}</div>
                  </td>
  ...
  function Legenda(){
    const items=[
      {icon:"🔄",label:"Próximo no rodízio",desc:"Avança para o próximo da fila"},
      {icon:"⭐",label:"Seleção especial",desc:"Especialista habilitada para atendimentos selecionados"},
      {icon:"🌴",label:"Férias",desc:"Fora do rodízio temporariamente por férias"},
      {icon:"⏸",label:"Pausado",desc:"Fora do rodízio por outro motivo"},
      {icon:"↩️",label:"Recart. Férias",desc:"Recarteirização temporária durante férias do titular"},
      {icon:"📌",label:"Indicação",desc:"Cliente indicado diretamente a este especialista"},
      {icon:"+1",label:"Cliente presencial",desc:"Cliente que escolheu ser atendido por quem está na agência"},
      {icon:"📝",label:"Nota",desc:"Anotação interna sobre o especialista"},
    ];
    return(
      <div style={{...C.card,marginTop:8,borderTop:"3px solid #7C3AED"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:8}}>
          {items.map(it=>(<div key={it.icon} style={{display:"flex",alignItems:"flex-start",gap:8,padding:"8px",background:"#f9fafb",borderRadius:10}}>
            <span style={{fontSize:18,flexShrink:0,minWidth:26,textAlign:"center"}}>{it.icon}</span>
  ...
  return(
    <div style={{fontFamily:f,background:"#f8f7ff",minHeight:"100vh",padding:"0.75rem"}}>
      ...
      {/* Modal editar contador (admin) */}
      {editCount&&<div style={{...C.card,borderLeft:"4px solid #7C3AED",marginBottom:16}}>
        <div style={{fontWeight:700,fontSize:14,marginBottom:12}}>✏️ Editar contador</div>
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          <input type="number" min="0" style={{...C.inp,maxWidth:100}} value={editCount.val} onChange={e=>setEditCount(p=>({...p,val:e.target.value}))} autoFocus onKeyDown={e=>e.key==="Enter"&&saveEditCount()}/>
          <button style={C.btnP} onClick={saveEditCount}>Salvar</button>
          <button style={C.btnS} onClick={()=>setEditCount(null)}>Cancelar</button>
        </div>
      </div>}
