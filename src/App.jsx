...
export default function App(){
  ...
  const [avulsoModal,setAvulsoModal]=useState(null);
  ...
  useEffect(()=>{
    ...
    const refreshAll=async()=>{
      try{
        const [sp,hi,ev,dc,la,sd,rm,bk,pr]=await Promise.all([
          ...
          sb("history?order=created_at.desc&limit=2000"),
        ...
        if(Array.isArray(ev))setEvts(ev);
        if(Array.isArray(dc))setDayLog(dc);
        if(Array.isArray(sd))setSdrs(sd);
        if(Array.isArray(rm))setRooms(rm);
        if(Array.isArray(bk))setBookings(bk);
        if(Array.isArray(pr))setPresence(pr);
        if(Array.isArray(la)){const lm={};la.forEach(r=>{lm[r.queue_id]={name:r.spec_name,type:r.type};});setLastMap(lm);}
    ...
    const channel=supabase.channel("rodizio-realtime")
  ...
  async function deleteHistoryRecord(h){
    if(h.date_key===today){
      const spec=specs.find(c=>c.name===h.spec_name);
      if(spec){
        const field=h.type==="indicacao"?"ind":"counts";
        const cur=spec[field]||{};
        const v=cur[h.queue_id]||0;
        if(v>0){await sb(`specialists?id=eq.${spec.id}`,"PATCH",{[field]:{...cur,[h.queue_id]:v-1}});}
      }
    }
    await sb(`history?id=eq.${h.id}`,"DELETE");
    const[hi2,sp]=await Promise.all([sb("history?order=created_at.desc&limit=2000"),sb("specialists?order=name")]);
    if(hi2)setHist(hi2);if(sp)setSpecs(sp);
...
async function iniciarNovoDia(){
  ...
  async function saveBooking(){
    ...
    try{
      const payload={room_id:parseInt(bkForm.room_id),room_name:room?.name||"",specialist_name:bkForm.specialist_name,booking_date:bkForm.booking_date,start_time,end_time,notes:bkNotes,booked_by:userName};
  ...
  function ControleTab(){
    ...
    const inQ=specs.filter(c=>{if(!c.queues.includes(ctrlQ))return false;if(!c.removed_at)return true;return new Date(c.removed_at)>=new Date(ctrlM.y,ctrlM.m,1);});
    ...
    function buildHTML(){
      const all=QUEUES.map(q=>{
        ...
        const inQq=specs.filter(c=>{if(!c.queues.includes(q.id))return false;if(!c.removed_at)return true;return new Date(c.removed_at)>=new Date(ctrlM.y,ctrlM.m,1);});
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
            ...
            <div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr auto 1fr auto 1fr auto 1fr",gap:4,alignItems:"center"}}>
                <select style={{...C.inp,padding:"10px 6px"}} value={bkForm.start_hour} onChange={e=>setBkForm(p=>({...p,start_hour:e.target.value}))}>{hours.map(h=><option key={h} value={h}>{h}</option>)}</select>
                <span style={{textAlign:"center",fontWeight:700}}>:</span>
                <select style={{...C.inp,padding:"10px 6px"}} value={bkForm.start_min} onChange={e=>setBkForm(p=>({...p,start_min:e.target.value}))}>{mins.map(m=><option key={m} value={m}>{m}</option>)}</select>
                <span style={{textAlign:"center",color:"#888",fontSize:12,padding:"0 2px"}}>até</span>
                <select style={{...C.inp,padding:"10px 6px"}} value={bkForm.end_hour} onChange={e=>setBkForm(p=>({...p,end_hour:e.target.value}))}>{hours.map(h=><option key={h} value={h}>{h}</option>)}</select>
                <span style={{textAlign:"center",fontWeight:700}}>:</span>
                <select style={{...C.inp,padding:"10px 6px"}} value={bkForm.end_min} onChange={e=>setBkForm(p=>({...p,end_min:e.target.value}))}>{mins.map(m=><option key={m} value={m}>{m}</option>)}</select>
              </div>
            </div>
          </div>
