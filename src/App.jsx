    ...
    return(
      <div style={{...C.card,borderTop:`4px solid ${q.color}`,padding:0,overflow:"hidden"}}>
        ...
        <div style={{padding:"0.75rem 1rem",borderBottom:"1px solid #f3f4f6",display:"flex",gap:8,flexWrap:"wrap"}}>
          ...
          <button style={{...C.btnS,flex:"1 1 100%",borderColor:"#10B981",color:"#10B981",fontWeight:600,fontSize:12}} onClick={()=>rotateRecart(qId)}>🌴 Recart. Férias</button>
        </div>
  ...
  function ControleTab(){
    ...
    return(
      <div>
          ...
          <table style={{borderCollapse:"collapse",minWidth:"100%"}}>
            <thead>
              <tr>
                ...
                {wd.map(d=><th key={d.toISOString()} style={{...th,minWidth:60}} colSpan={2}>{d.toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit"})}</th>)}
                <th style={{...th,background:qInfo.light,color:qInfo.color}} colSpan={2}>Total</th>
              </tr>
              <tr>
                {wd.map(d=>[<th key={d.toISOString()+"n"} style={{...th,fontSize:10,color:"#7C3AED"}}>N</th>,<th key={d.toISOString()+"i"} style={{...th,fontSize:10,color:"#F59E0B"}}>I</th>])}
        ...
        <div style={{marginTop:8,fontSize:12,color:"#888"}}>N = rodízio normal · I = indicações + seleção</div>
      </div>
    );
  }
      ...
      {adminOpen&&(
        <div style={{background:"#fff",borderRadius:16,padding:"1.5rem",border:"1px solid #e5e7eb",marginBottom:16,boxShadow:"0 4px 24px #7C3AED20"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            <button style={{padding:"8px 12px",borderRadius:8,border:"1px solid #FECACA",background:"#FEE2E2",color:"#EF4444",fontSize:12,fontWeight:600,cursor:"pointer",textAlign:"left"}} onClick={async()=>{if(!confirm("Limpar todo o histórico de rodízio?"))return;await sb("history","DELETE");setHist([]);showToast("Histórico limpo!");}}>🗑️ Limpar histórico de rodízio</button>
            <button style={{padding:"8px 12px",borderRadius:8,border:"1px solid #FECACA",background:"#FEE2E2",color:"#EF4444",fontSize:12,fontWeight:600,cursor:"pointer",textAlign:"left"}} onClick={async()=>{if(!confirm("Limpar todos os eventos e pausas?"))return;await sb("events","DELETE");setEvts([]);showToast("Eventos limpos!");}}>🗑️ Limpar pausas e eventos</button>
            <button style={{padding:"8px 12px",borderRadius:8,border:"1px solid #FECACA",background:"#FEE2E2",color:"#EF4444",fontSize:12,fontWeight:600,cursor:"pointer",textAlign:"left"}} onClick={async()=>{if(!confirm("Limpar todas as reservas de salas?"))return;await sb("meeting_bookings","DELETE");setBookings([]);showToast("Reservas limpas!");}}>🗑️ Limpar reservas de salas</button>
            <button style={{padding:"8px 12px",borderRadius:8,border:"1px solid #FECACA",background:"#FEE2E2",color:"#EF4444",fontSize:12,fontWeight:600,cursor:"pointer",textAlign:"left"}} onClick={async()=>{if(!confirm("Limpar todo o calendário de presença?"))return;await sb("presence_calendar","DELETE");setPresence([]);showToast("Presença limpa!");}}>🗑️ Limpar calendário de presença</button>
            <button style={{padding:"8px 12px",borderRadius:8,border:"1px solid #FECACA",background:"#FEE2E2",color:"#EF4444",fontSize:12,fontWeight:600,cursor:"pointer",textAlign:"left"}} onClick={async()=>{if(!confirm("Limpar fechamentos de dia?"))return;await sb("day_closings","DELETE");setDayLog([]);showToast("Fechamentos limpos!");}}>🗑️ Limpar fechamentos de dia</button>
            <button style={{padding:"8px 12px",borderRadius:8,border:"2px solid #EF4444",background:"#FEE2E2",color:"#EF4444",fontSize:12,fontWeight:700,cursor:"pointer",textAlign:"left"}} onClick={async()=>{if(!confirm("⚠️ ATENÇÃO: Isso vai apagar TODOS os dados (histórico, reservas, presença, eventos, fechamentos e zerar contadores). Tem certeza?"))return;await Promise.all([sb("history","DELETE"),sb("events","DELETE"),sb("meeting_bookings","DELETE"),sb("presence_calendar","DELETE"),sb("day_closings","DELETE"),sb("last_assigned","DELETE")]);for(const s of specs)await sb(`specialists?id=eq.${s.id}`,"PATCH",{counts:{},ind:{}});setHist([]);setEvts([]);setBookings([]);setPresence([]);setDayLog([]);setLastMap({});const sp=await sb("specialists?order=name");if(sp?.length)setSpecs(sp);showToast("Todos os dados foram apagados!");}}>⚠️ Apagar TUDO (reset completo)</button>
          </div>
        </div>
      )}
      {modal&&(
